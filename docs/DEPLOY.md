...
`deploy:web` defaults `VITE_PARTYKIT_HOST` to `sixseven.kasusoba.partykit.dev`.
Forkers override it without editing tracked files:

```bash
VITE_PARTYKIT_HOST=sixseven.<your-username>.partykit.dev pnpm deploy:web
```
